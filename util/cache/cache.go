...
func getFlagVal(cmd *cobra.Command, o Options, name string, getVal func(name string) (interface{}, error)) func() interface{} {
...
func getStrFlagVal(cmd *cobra.Command, o Options, name string, getVal func(name string) (string, error)) func() string {
	fn := getFlagVal(cmd, o, name, func(name string) (interface{}, error) { return getVal(name) })
	return func() string { return fn().(string) }
}

func getBoolFlagVal(cmd *cobra.Command, o Options, name string, getVal func(name string) (bool, error)) func() bool {
	fn := getFlagVal(cmd, o, name, func(name string) (interface{}, error) { return getVal(name) })
	return func() bool { return fn().(bool) }
}

func getIntFlagVal(cmd *cobra.Command, o Options, name string, getVal func(name string) (int, error)) func() int {
	fn := getFlagVal(cmd, o, name, func(name string) (interface{}, error) { return getVal(name) })
	return func() int { return fn().(int) }
}

func getDurationFlagVal(cmd *cobra.Command, o Options, name string, getVal func(name string) (time.Duration, error)) func() time.Duration {
	fn := getFlagVal(cmd, o, name, func(name string) (interface{}, error) { return getVal(name) })
	return func() time.Duration { return fn().(time.Duration) }
}

func getStringArrayFlagVal(cmd *cobra.Command, o Options, name string, getVal func(name string) ([]string, error)) func() []string {
	fn := getFlagVal(cmd, o, name, func(name string) (interface{}, error) { return getVal(name) })
	return func() []string { return fn().([]string) }
}
...
func AddCacheFlagsToCmd(cmd *cobra.Command, opts ...Options) func() (*Cache, error) {
	...
	redisAddressSrc := getStrFlagVal(cmd, opt, "redis", cmd.Flags().GetString)
	...
	redisDBSrc := getIntFlagVal(cmd, opt, "redisdb", cmd.Flags().GetInt)
	...
	sentinelAddressesSrc := getStringArrayFlagVal(cmd, opt, "sentinel", cmd.Flags().GetStringArray)
	cmd.Flags().StringVar(&sentinelMaster, opt.FlagPrefix+"sentinelmaster", "master", "Redis sentinel master group name.")
	sentinelMasterSrc := getStrFlagVal(cmd, opt, "sentinelmaster", cmd.Flags().GetString)
	...
	defaultCacheExpirationSrc := getDurationFlagVal(cmd, opt, "default-cache-expiration", cmd.Flags().GetDuration)
	cmd.Flags().BoolVar(&redisUseTLS, opt.FlagPrefix+"redis-use-tls", false, "Use TLS when connecting to Redis. ")
	redisUseTLSSrc := getBoolFlagVal(cmd, opt, "redis-use-tls", cmd.Flags().GetBool)
	...
	redisClientCertificateSrc := getStrFlagVal(cmd, opt, "redis-client-certificate", cmd.Flags().GetString)
	...
	redisClientKeySrc := getStrFlagVal(cmd, opt, "redis-client-key", cmd.Flags().GetString)
	...
	insecureRedisSrc := getBoolFlagVal(cmd, opt, "redis-insecure-skip-tls-verify", cmd.Flags().GetBool)
	...
	redisCACertificateSrc := getStrFlagVal(cmd, opt, "redis-ca-certificate", cmd.Flags().GetString)
	cmd.Flags().StringVar(&compressionStr, opt.FlagPrefix+"redis-compress", env.StringFromEnv(opt.getEnvPrefix()+"REDIS_COMPRESSION", string(RedisCompressionGZip)), "Enable compression for data sent to Redis with the required compression algorithm. (possible values: gzip, none)")
	compressionStrSrc := getStrFlagVal(cmd, opt, "redis-compress", cmd.Flags().GetString)
